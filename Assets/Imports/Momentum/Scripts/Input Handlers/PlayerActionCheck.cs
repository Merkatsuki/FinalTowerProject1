...
namespace Momentum 
{
	public class PlayerActionCheck
	{
		...
		public bool Fly(PlayerBaseState state) 
		{
			...
			{
				if (!_playerReferences.PChecks.IsGrounded && _playerReferences.PRB.linearVelocity.y > 0 && !_playerReferences.PChecks.IsTouchingFacingWall) { return true; } // Not grounded and Y velocity is less than 0 and not touching the wall we are facing
				else { return false; }
			} else if (state.GetType().IsSubclassOf(typeof(PlayerGroundedState))) // Is the state we are in a subclass of grounded state
			{
				if (!_playerReferences.PChecks.IsGrounded && _playerReferences.PRB.linearVelocity.y > 0) { return true; } // Not grounded and y velocity is less than 0
				else { return false; }
			}
			
			if (_playerReferences.PRB.linearVelocity.y > 0) { return true; } // Else Y velocity is less than 0
		...
		public bool Fall(PlayerBaseState state) 
		{
			...
			{
				if (!_playerReferences.PChecks.IsGrounded && _playerReferences.PRB.linearVelocity.y <= 0 && !_playerReferences.PChecks.IsTouchingFacingWall) { return true; } // Not grounded and velocity is less than or equal to 0 and we aren't touching the wall we are facing
				else { return false; }
			...
			if (_playerReferences.PRB.linearVelocity.y <= 0) { return true; } // If none of the above, we are below Y velocity of 0
