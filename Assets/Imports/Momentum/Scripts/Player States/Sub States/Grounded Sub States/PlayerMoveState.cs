	...
	public class PlayerMoveState : PlayerGroundedState
	{
		...
		public override void PhysicsUpdate()
		{
			...
			if (_playerData.AddForceParallelToSlope && _playerChecks.GroundedCheckCtx.normal.x % 1 != 0 && _playerChecks.GroundedCheckCtx.normal.y % 1 != 0) // Apply Force Parallel to Ground and ground is not flat
			{
				...
				_playerReferences.PRB.linearVelocity += (Vector2) _directionOfForce * (Time.deltaTime * _finalMovement); // Apply move speed parallel to ground using the direction vector
			} else
			{
				...
				_playerReferences.PRB.linearVelocity += (Time.deltaTime * _finalMovement * Vector2.right); // Apply it
			}
		}
	}
}