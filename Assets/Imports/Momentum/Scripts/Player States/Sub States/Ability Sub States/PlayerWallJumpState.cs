...
namespace Momentum.States
{
	public class PlayerWallJumpState : PlayerAbilityState
	{
		...
		public override void LogicUpdate()
		{
			...
			if (_playerReferences.PRB.linearVelocity.y < _playerData.WallJumpYThresh && _timeInState > _playerData.WallJumpTime && !_isAbilityDone) // If Y velocity is below threshold and we have wall jumped for the minimum amount of time
			{
		...
		public override void PhysicsUpdate()
		{
			...
			_speedDif = _targetSpeed - (_targetSpeed == 0 ? 0 : _playerReferences.PRB.linearVelocity.x);
			_finalMovement = Mathf.Pow(Mathf.Abs(_speedDif) * _playerData.Acceleration, _playerData.VelocityPower) * Mathf.Sign(_speedDif);
			
			_playerReferences.PRB.linearVelocity += (Time.deltaTime * (_finalMovement * _movementControl)) * Vector2.right;
		...
		{
			_playerReferences.PRB.linearVelocity = new Vector2(_playerData.LinearWallJumpForce.x * _facingInteger, _playerData.LinearWallJumpForce.y); // Add the force
		}
