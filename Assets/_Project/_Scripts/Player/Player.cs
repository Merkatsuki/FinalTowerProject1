...
namespace Momentum
{
    public class Player : MonoBehaviour, IPuzzleInteractor
    {
        ...
        public IEnumerator ReenableOneWayCollision(Collider2D platformCollider)
        {
            yield return new WaitForSeconds(0.5f); // Enough to fully drop through
            Physics2D.IgnoreCollision(_playerReferences.PBC, platformCollider, false);
        }
        public void TriggerFallThrough(Collider2D platformCollider)
        {
            Physics2D.IgnoreCollision(_playerReferences.PBC, platformCollider, true);
            StartCoroutine(ReenableOneWayCollision(platformCollider));
        }
