...
public class PlayerInteractor : MonoBehaviour
{
    ...
    private void UpdateFacingDirection()
    {
        ...
        Vector2 direction = (mouseWorldPos - facingDirection.position).normalized;
        if (direction.sqrMagnitude > 0.001f)
        {
            facingDirection.right = direction;
        }
    }
    ...
    {
        float dot = Vector2.Dot(facingDirection.right, toTarget.normalized);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Vector3 origin = facingDirection.position;
        Vector3 forward = facingDirection.right;
