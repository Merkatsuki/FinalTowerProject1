...
namespace Momentum.States
{
    public class PlayerJumpState : PlayerAbilityState
    {
        ...
        public override void Enter()
        {
            ...
            _playerReferences.PRB.linearVelocity = new Vector2(_playerReferences.PRB.linearVelocity.x, _initVelocity); // Add the inital jump velocity to player
        }
        ...
        public override void LogicUpdate()
        {
            ...
            if (InputManager.instance.JumpPressedThisFrame && _playerReferences.PChecks.TryFallThroughOneWay())
            {
            ...
            if (_timeInState > _playerData.MinJumpTime && _playerReferences.PRB.linearVelocity.y < _playerData.JumpYThresh) { EndJump(); } // Jumped for minimum required time and passed under Y threshold in velocity
        ...
        public override void PhysicsUpdate()
        {
            ...
            if (_playerReferences.PChecks.TryFallThroughOneWay())
                return;
            ...
            _speedDif = _targetSpeed - _playerReferences.PRB.linearVelocity.x;
            _finalMovement = Mathf.Pow(Mathf.Abs(_speedDif) * _playerData.Acceleration, _playerData.VelocityPower) * Mathf.Sign(_speedDif);

            _playerReferences.PRB.linearVelocity += (Time.deltaTime * _finalMovement) * Vector2.right;
        ...
        private void EndJump() // Called to end jump for reasons other than released jump, can be called without variable jump height enabled
        {
            ...
            { // Jump Cut
                _isAbilityDone = true;
                _playerReferences.PRB.linearVelocity = new Vector2(_playerReferences.PRB.linearVelocity.x, _playerReferences.PRB.linearVelocity.y > 0.2 ? 1f : _playerReferences.PRB.linearVelocity.y);
        ...
        public void ReleasedJump() // Called only when jump is released
        {
            ...
            { // Jump Cut
                _isAbilityDone = true;
                _playerReferences.PRB.linearVelocity = new Vector2(_playerReferences.PRB.linearVelocity.x, _playerReferences.PRB.linearVelocity.y > 0.5 ? _playerData.JumpCutVelocity : _playerReferences.PRB.linearVelocity.y);
                if (_player.CanDoAction.Fall(this)) { _stateMachine.ChangeState(_player.FallState); return; }
                if (_player.CanDoAction.Fly(this)) { _stateMachine.ChangeState(_player.FlyState); return; }
            }
        }
