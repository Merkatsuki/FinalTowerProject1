...
namespace Momentum
{
    public class PlayerChecks : MonoBehaviour
    {
        ...
        public Collider2D LastOneWayPlatformCollider { get; private set; }
        ...
        {
            // Ensure it's still touching ground and that ground was a one-way platform
            return GroundedCheckCtx.collider != null &&
                   (_playerReferences.PData.OneWayPlatformLayerMask.value & (1 << GroundedCheckCtx.collider.gameObject.layer)) != 0;
        }

        public bool TryFallThroughOneWay()
        {
            if (!InputManager.instance.IsPressingDown)
                return false;

            bool onGround = IsStandingOnOneWayPlatform();
            Collider2D belowPlatform = null;

            if (!_playerReferences.PChecks.IsGrounded)
            {
                belowPlatform = GetOneWayPlatformBelowWhileFalling();
                if (belowPlatform != null)
                    LastOneWayPlatformCollider = belowPlatform;
            }

            if ((onGround || belowPlatform != null) && LastOneWayPlatformCollider != null)
            {
                _player.StartCoroutine(FallThroughRoutine(LastOneWayPlatformCollider));
                InputManager.instance.BlockJumpUntilRelease();
                return true;
            }
        ...
        private IEnumerator FallThroughRoutine(Collider2D platform)
        {
            yield return new WaitForFixedUpdate(); // 🔧 wait one physics frame to prevent re-grounding before ignore applies
            Physics2D.IgnoreCollision(_playerReferences.PBC, platform, true);
            yield return new WaitForSeconds(0.5f);
            Physics2D.IgnoreCollision(_playerReferences.PBC, platform, false);
        }



        public Collider2D GetOneWayPlatformBelowWhileFalling()
        {
            Vector2 origin = new Vector2(_bcPos.x, _bcPos.y - _bcExtents.y);
            Vector2 size = new Vector2(_bcExtents.x * 2, 0.05f); // Thin box just beneath feet
            ...
            RaycastHit2D hit = Physics2D.BoxCast(
                ...
                Vector2.down,
                0.05f,
                _playerReferences.PData.OneWayPlatformLayerMask
            );
        ...
        {
            GroundedCheckCtx = Physics2D.BoxCast(
                new Vector2(_bcPos.x, _bcPos.y - _bcExtents.y - (_playerReferences.PData.GroundedCheckBoxLength * 0.5f)),
                new Vector2((_bcExtents.x * 2) - _playerReferences.PData.GroundedCheckBoxShrinkX, _playerReferences.PData.GroundedCheckBoxLength),
                0f,
                Vector2.down,
                0f,
                _playerReferences.PData.GroundLayerMask | _playerReferences.PData.OneWayPlatformLayerMask
            );

            if (GroundedCheckCtx.collider != null)
            {
                int layer = GroundedCheckCtx.collider.gameObject.layer;
                bool isOneWay = (_playerReferences.PData.OneWayPlatformLayerMask.value & (1 << layer)) != 0;

                // Always store reference if touching a one-way; clear if not.
                LastOneWayPlatformCollider = isOneWay ? GroundedCheckCtx.collider : null;

                return true;
            }

            LastOneWayPlatformCollider = null;
            return false;
        }
