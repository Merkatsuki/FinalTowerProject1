...
namespace Momentum.States
{
    public class PlayerGroundedState : PlayerBaseState
    {
        ...
        public override void LogicUpdate()
        {
            ...
            // Add Friction
            if (!_player.CanDoAction.Move(this) && _playerReferences.PRB.linearVelocity.x != 0)
            {
                _minAmount = Mathf.Min(Mathf.Abs(_playerReferences.PRB.linearVelocity.x), Mathf.Abs(_playerData.GroundFrictionAmount)); // Check if our velocity, or friction amount is smaller
                _minAmount *= Mathf.Sign(_playerReferences.PRB.linearVelocity.x); // Reapply direction
        ...
        public override void PhysicsUpdate()
        {
            ...
            if (_playerData.PreventBouncingOnSlope) // If disable bouncing
            {
                ...
                if (StickToGround.distance < StickToGround2.distance && StickToGround.distance > 0.05) { _playerReferences.PRB.linearVelocity -= new Vector2(0, _playerData.AntiBounceForce); }
                if (StickToGround2.distance < StickToGround.distance && StickToGround2.distance > 0.05) { _playerReferences.PRB.linearVelocity -= new Vector2(0, _playerData.AntiBounceForce); }
