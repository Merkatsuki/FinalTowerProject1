...
namespace Momentum.States
{
	public class PlayerDashState : PlayerAbilityState
	{
		...
		public override void Exit()
		{
			...
			_playerReferences.PRB.linearVelocity = new Vector2(_playerReferences.PRB.linearVelocity.x, -0.15f);
		...
		public override void LogicUpdate()
		{
			...
			if (_isAbilityDone)
			{
				if (_dashHangTime > 0) // Do hang time then transition to next state
				{
					...
					_playerReferences.PRB.linearVelocity = new Vector2(_playerReferences.PRB.linearVelocity.x, Mathf.SmoothDamp(_playerReferences.PRB.linearVelocity.y, 0, ref _dampYToZero, _playerData.DashHangTime));
					if (_setYVelocityToZero) { _playerReferences.PRB.linearVelocity = new Vector2(_playerReferences.PRB.linearVelocity.x, 0); }
		...
		private void Dash(Vector2 DashDirection) 
		{
			...
			_playerReferences.PRB.linearVelocity = DashDirection * _currentStrength; // Extend the normalized dash direction vector by multiplying the desired length (strength)
		}
