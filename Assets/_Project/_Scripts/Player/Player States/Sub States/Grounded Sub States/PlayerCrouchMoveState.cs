	...
	public class PlayerCrouchMoveState : PlayerGroundedState
	{
		...
		public override void PhysicsUpdate()
		{
			...
			{ // If we are on slope then apply force parallel
				...
				_playerReferences.PRB.linearVelocity = _directionOfForce * _playerData.CrouchMoveSpeed; // Apply the force in that direction
			} else
			{ // Else apply it on the X axis
				_playerReferences.PRB.linearVelocity = new Vector2(_wasdInput.x * _playerData.CrouchMoveSpeed, _playerReferences.PRB.linearVelocity.y); // Add the force parallel to the X axis
			}
		}
